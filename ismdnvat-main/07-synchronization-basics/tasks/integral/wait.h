...
inline void atomic_wait_explicit(_Atomic(uint32_t)* ptr, uint32_t old, int mo) {
  while (atomic_load_explicit(ptr, mo) == old) {
    PlatformWait((uint32_t*)ptr, old);
  }
}

inline void atomic_wait(_Atomic(uint32_t)* ptr, uint32_t old) {
  return atomic_wait_explicit(ptr, old, __ATOMIC_SEQ_CST);
}

inline void atomic_notify_one(_Atomic(uint32_t)* ptr) {
  PlatformWakeOne((uint32_t*)ptr);
}

inline void atomic_notify_all(_Atomic(uint32_t)* ptr) {
